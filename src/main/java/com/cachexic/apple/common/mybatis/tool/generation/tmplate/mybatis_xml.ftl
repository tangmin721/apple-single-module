...
<mapper namespace="${CONFIG.packagePath}.dao.${entity.className}Dao">
	...
	<resultMap id="${entity.firstLowName}" type="${CONFIG.packagePath}.entity.${entity.className}">
<#list entity.allfieldList as e>		<result property="${e.fieldName}" column="${e.columnName}" />
		</#list>
	</resultMap>
	
	<!--单条插入-->
	<insert id="insert" parameterType="${CONFIG.packagePath}.entity.${entity.className}" keyProperty="id" useGeneratedKeys="true">
		insert into
		<include refid="table" />
...
<#list entity.myfieldList as e>			<if test="${e.fieldName} != null">${r"#{"}${e.fieldName}${r"}"}<#if e_has_next>,</#if></if>
	...
	<update id="update" parameterType="${CONFIG.packagePath}.entity.${entity.className}">
		update
		<include refid="table" />
		<set>
...
<#list entity.myfieldList as e>			<if test="${e.fieldName} != null and ${e.fieldName} !=''"> 
	...
	<!--查询所有记录-->
	<select id="selectList" parameterType="${CONFIG.packagePath}.entity.${entity.className}Query" resultMap="${entity.firstLowName}">
	...
	<!-- 分页 -->
	<select id="selectListPage" parameterType="${CONFIG.packagePath}.entity.${entity.className}Query" resultMap="${entity.firstLowName}">
		<include refid="selectorFields" />
		<include refid="selectorWhere" />
		<include refid="selectorOrder" />
	...
	<!--总条数-->
	<select id="selectListTotal" parameterType="${CONFIG.packagePath}.entity.${entity.className}Query" resultType="long">
